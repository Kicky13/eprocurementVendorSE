        ...
        <div class="modal-content" >
            ...
            <div class="modal-body">
                <div id="proc_msg" class="col-12">
                    <p><?=lang("Anda akan melakukan proposal lelang","You will be developing Bid Proposal")?></p>
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-default" data-dismiss="modal"><?=lang("Batal","cancel")?></button>
                <button class="btn btn-primary" id="snd_join" onclick="send()"><?=lang("Ya, Lanjut","Yes Confirmation ")?></button>                
            </div>
        </div>
    </div>
</div>
...
$(function(){
    ...
    var tabel = $('#datalelang').DataTable({        
        ajax: {
            url: '<?= base_url('vn/info/bid_submission/show_list_invite') ?>',
            'dataSrc': ''
        },
        "scrollX": true,
        "selected": true,
...
function process()
{    
    $('#modal_process .modal-title').html("<?=lang('Konfirmasi','Confirmation')?>");            
...
function modal_komen(type) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?= base_url('vendor/bid_submission/comment/') ?>',
        data: obj,
        success: function (m) {                    
            $('#message').html(m);
            $('#type_msg').val(type)
...
function get_tampilan(addr,idx,dt)
{
    ...
    $.ajax({
        type: 'POST',
        url: '<?= base_url('vn/info/bid_submission/') ?>'+addr,        
        data:obj,
        success: function (m) 
        {
            stop(elm);            
...
{        
    var tabel2 = $('#attch').DataTable({        
        ajax: {
            url: '<?= base_url('vn/info/bid_submission/show_attch/') ?>'+id,
            'dataSrc': ''
        },
        "scrollX": true,
        "selected": true,
