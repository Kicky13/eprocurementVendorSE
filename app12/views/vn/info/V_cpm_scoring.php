            ...
            <section>
                <div class="card">
                    <div class="card-header">                        
                        <div class="row">
                            <div class="col-sm-8">    
                                <h3><?=lang("Penilaian CPM","CPM Scoring")?></h3>
                            </div>                            
                        </div>                 
                    </div>
                    <div class="card-body">                                                    
                ...
                <div class="card">
                    <div class="card-header" style="background-color:#d3f7ef">                        
                        <div class="col-12 row">
                            <div class="col-8">                            
                                <h3 class="content-header-title mb-1">CPM Scoring <span id="company"></span></h3>                                                                    
                            </div>
                            <div class="col-4">                            
                                            ...
                                            <table id="kpi" class="table table-striped table-bordered table-hover display text-center" width="100%">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th style="vertical-align: middle; min-width: 250px;" colspan="2">Contractor Self Scoring</th>
                        ...
                        <div class="card-body">
                            ...
                            <div class="col-12 row">
                                ...
                                <div class="col-8 text-right">
                                    <button class="btn btn-primary" id="draft"><?= lang("Simpan Draft","Save Draft")?></button>
                                    <button class="btn btn-success" id="send"><?= lang("Kirim","Submit")?></button>                                    
                                </div>                                                                
                            </div>
                        </div>
                    </div>
...
$(function(){                 
    ...
    ajax: {
        url: '<?= base_url('vn/info/cpm_scoring/get_list/') ?>',
        'dataSrc': ''
    },
    "scrollX": true,
    "selected": true,
    ...
    $("#draft_mdl").submit(function (){
        ...
        obj.phase=$('.phase').val();     
        $.ajax({
            url:"<?= base_url("vn/info/cpm_scoring/draft/")?>",
            type:"POST",
            data:obj,
            success:function(m){
                if(m.status === "Success")
    ...
    $("#submit_mdl").submit(function (){
        ...
        obj.phase=$('.phase').val();
        $.ajax({
            url:"<?= base_url("vn/info/cpm_scoring/send/")?>",
            type:"POST",
            data:obj,
            success:function(m){
                if(m.status === "Success")
...
function table_weight() {
    ...
    obj.phase = $('.phase').val();
    $.ajax({
        url: "<?=base_url('vn/info/cpm_scoring/get_cpm_detail')?>",
        data: obj,
        type: "POST",
        success: function(m) {
            if (m.length == 0)
                ...
                $.each(m,function(i,item) {
                    iw_base = (item.act_weight == null ? '0.00' : (item.act_weight*1).toFixed(2));
                    ...
                    if (tamp == 0) {
                        ...
                        st_kpi_as += (item.act_score * 1);
                        st_kpi_aws += (item.act_weight * 1);
                        ...
                        ("<tr>"
                            ...
                            +"<td><input type='number' onchange='calc("+item.id+", "+item.category_id+")' class='form-control' min=0 max="+item.target_score+" name='"+item.id+"_0' value='"+item.act_score+"' /></td>"
                            +"<td><p id='iw_mask_"+item.id+"'>"+iw_base+"</p><input type='hidden' class='form-control' name='"+item.id+"_1' value='"+iw_base+"' /></td>"
                            +"<td><input type='text' name='"+item.id+"_r' value='"+item.remarks+"' /></td>"
                        +"</tr>"
                        );
                        ...
                        ("<tr>"
                            ...
                            +"<td id='stas_"+tamp+"'>"+st_kpi_as+"</td>"
                            +"<td id='staws_"+tamp+"'>"+st_kpi_aws.toFixed(2)+"</td>"
                            +"<td></td>"
                        +"</tr>"
                        );
                ...
                ("<tr>"
                    ...
                    +"<td id='gtas'>"+gt_kpi_as+"</td>"
                    +"<td id='gtaws'>"+gt_kpi_aws.toFixed(2)+"</td>"
                    +"<td></td>"
                +"</tr>"
                );
            }
...
function calc(id, cat) {
    var val = $('input[name='+id+'_0]').val();
    var max = ($('input[name='+id+'_0]').prop('max') * 1);
    var min = ($('input[name='+id+'_0]').prop('min') * 1);
    var obj = {};
    obj.po = $('#po_id').val();
    if (val > max) {
        obj.val = max;
        $('input[name='+id+'_0]').val(max);
    } else if (val < min){
        obj.val = min;
        $('input[name='+id+'_0]').val(min);
    } else {
        obj.val = val;
    }

    $.ajax({
        url: "<?= base_url('vn/info/cpm_scoring/calc/')?>"+id,
        type: "POST",
        data: obj,
        success: function (m) {
            if (!m.status) {
                ...
                $.each(m, function(i, item) {
                    if (item.id == id) {
                        $('input[name='+id+'_1]').val(item.weight);
                        $('#iw_mask_'+id).html(item.weight);
                    }
                    if (tcat != 0 && tcat != item.category_id) {
                        $('#stas_'+tcat).html(ts);
                        $('#staws_'+tcat).html(tw.toFixed(2));
                        gs += ts;
                        gw += tw;
                        ts = 0;
                        tw = 0;
                    }
                    ts += ($('input[name='+item.id+'_0]').val() * 1);
                    tw += ($('input[name='+item.id+'_1]').val() * 1);
...
function tbl()
{
    ...
    ajax: {
        url: '<?= base_url('vn/info/cpm_scoring/get_upload/') ?>'+po,
        'dataSrc': ''
    },
    "scrollX": true,
    "selected": true,
