    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12 mb-1">
                <h3 class="content-header-title"><?= lang("Service receive notification", "Service receive notification") ?></h3>
            </div>
            <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-12">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                      ...
                      <li class="breadcrumb-item active"><?= lang("Service Receive Notification", "Service Receive Notification") ?></li>
                    </ol>
                </div>
            </div>
        </div>
                                            ...
                                            <table id="tbl" class="table table-striped table-bordered table-fixed-column order-column dataex-lr-fixedcolumns table-hover display" width="100%">
                                                <tfoot>
                                                    <tr>
                                                        ...
                                                        <th><center><?= lang("Tgl Dibuat", "Created Date") ?></center></th>
                                            ...
                                            <table id="tbl1" class="table table-striped table-bordered table-fixed-column order-column dataex-lr-fixedcolumns table-hover display" width="100%">
                                                <tfoot>
                                                    <tr>
                                                        ...
                                                        <th><center><?= lang("Vendor", "Vendor") ?></center></th>
        ...
        <div class=" modal-content">
              ...
              <div class="modal-header">
                        ...
                        <div class="card-body">
                          ...
                          <table class="table table-responsive">
                            <tbody>
                              ...
                              <tr>
                                <th class="text-nowrap" scope="row">Email Vendor</th>
                                <td><code><?= $this->session->ID_VENDOR; ?></code></td>
                              </tr>
                              <tr>
                                <th class="text-nowrap" scope="row">ITP No</th>
                                ...
                                <div id="edit" class="wrapper wrapper-content animated fadeInRight white-bg">
                                  <form id="itp_form" action="#" class="steps-validation wizard-circle" enctype="multipart/form-data">
                                      ...
                                      <fieldset id="bagian1" class="col-md-12" class="white-bg">
                                          ...
                                          <input type="hidden" id="id_itp_vendor" name="id_itp_vendor" value="">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id="save"><?= lang('Accept', 'Accept') ?></button>
                </div>
              </form>
        </div>
    </div>
        ...
        <form class=" modal-content" id="reject_mdl">
            ...
            <div class="modal-body">
                <div class="form-horizontal">
                    ...
                    <input type="hidden" name="id_itp_vendor_rej" id="id_itp_vendor_rej" value="">
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $(document).on('show.bs.modal', '#myModal', function(e){
      ...
      $("#id_itp_vendor").attr('disabled', false);
      ...
      $("#id_itp_vendor").val(idnya);

      // alert(idnya);
      $.ajax({
        url: '<?= base_url('vn/info/Service_receive_notification/get_item_selection') ?>',
        type: 'POST',
        dataType: 'JSON',
        data: {idnya: idnya}
      })
      ...
      .always(function(res) {
        ...
        if (result == true) {
          ...
          $.each(res, function(index, el) {
            ...
            total_spending.push(speding);
            total_remaining.push(parseInt(total_harga));
                str = '<tr>'+
                        '<td>'+no+'</td>'+
                        '<td>'+el.MATERIAL_NAME+'</td>'+
                        '<input type="hidden" id="qty'+el.MATERIAL+'" value="'+el.qty+'" />'+
                        '<td>'+el.priceunit+'</td>'+
                        ...
                        '<td><input id="spending'+el.MATERIAL+'" style="width: fit-content;" class="form-control tdspending" type="text" value="'+speding+'" readonly /></td>'+
                        '<td><input id="total_spending'+el.MATERIAL+'" style="width: fit-content;" class="form-control tdremaining" name="total_spending[]" type="text" value="'+parseInt(total_harga)+'" readonly /></td>'+
            ...
            setTimeout(function(){
              if (al[4] == 1) {
                ...
                $(".remove_item"+el.MATERIAL).prop('disabled', false);
                $(".checklist_item"+el.MATERIAL).prop('disabled', false);
                $("#spending2"+el.MATERIAL).prop('disabled', false);
                $(".remove_item"+el.MATERIAL).prop('disabled', false);
                $("#descTextarea").prop('disabled', false);
                console.log("on");
              } else {
                console.log("off");
                ...
                $(".remove_item"+el.MATERIAL).prop('disabled', true);
                $("#descTextarea").prop('disabled', true);
              }
            },100);
          });
      ...
      // show itp data
      $.ajax({
        url: '<?=base_url('vn/info/Service_receive_notification/show_data_itp')?>',
        type: 'POST',
        dataType: 'json',
        data: {msr_no: idnya}
      })
    ...
    $("#itp_form").submit(function(e){
      ...
      // console.log(data);
      $.ajax({
        url: '<?= base_url('vn/info/Service_receive_notification/itp_doc_approval')?>',
        type: 'POST',
        dataType: 'JSON',
        data: data,
        cache: false,
      ...
      .always(function(res) {
        if (resultx == true) {
          ...
          swal({
                ...
                window.location.href = "<?= base_url()?>/vn/info/Service_receive_notification";
          });
        } else {
          modal_stop(xmodalx);
          window.location.href = "<?= base_url()?>/vn/info/Service_receive_notification";
    ...
    $(document).on('show.bs.modal', '#modal_rej', function(e) {
      $("#id_rej").prop("disabled", false);
      $("#id_itp_rej").prop("disabled", false);
      $("#user_roles_rej").prop("disabled", false);
      $("#email_approve_rej").prop("disabled", false);
      $("#edit_content_rej").prop("disabled", false);
      $("#email_reject_rej").prop("disabled", false);
      $("#sequence_rej").prop("disabled", false);
      $("#status_approve_rej").prop("disabled", false);
      $("#reject_step_rej").prop("disabled", false);
      $("#id_itp_vendor_rej").prop("disabled", false);
      ...
      $("#reject_mdl").submit(function(e) {
        ...
        $.ajax({
          url: '<?= base_url("vn/info/Service_receive_notification/reject_itp_doc")?>',
          type: 'POST',
          dataType: 'JSON',
          data: data,
          cache: false,
    ...
    // $(document).on('click', '#Rejcet', function(e) {
    //   // e.preventDefault();
    //   /* Act on the event */
    //   var idnya = $(this).data("id");
    //
    ...
    //           text: "ITP Document Will Be Rejected!"+idnya,
    //           type: "warning",
    //           showCancelButton: true,
    //           confirmButtonColor: "#d63035",
    ...
    $(document).on('click', '#print_sr', function(e) {
      // e.preventDefault();
      /* Act on the event */
      var idnya = $(this).data("id");
      window.open('<?= base_url("vn/info/Print_sr/print_sr_priview/")?>'+idnya);
      // alert("PRINT SR "+idnya)
    });
    ////////////////////////////////////////////////////////////////// hole process /////////////////////////////////////////////////////////////////////////////
    ...
    var table=$('#tbl1').DataTable({
        "ajax": {
            "url": "<?= base_url('vn/info/Service_receive_notification/datatable_msr_remaining') ?>",
            "dataSrc": ""
        },
        "data": null,
        "searching": true,
    ...
    var table=$('#tbl').DataTable({
        "ajax": {
            "url": "<?= base_url('vn/info/Service_receive_notification/datatable_list_itp_on_progress') ?>",
            "dataSrc": ""
        },
        "data": null,
        "searching": true,
  ...
  function get_id(id, type, desc, qty, uom, unitprice, total_harga, no, remark){
    ...
    var str = '<tr>'+
                ...
                '<input style="width: fit-content;" class="form-control" id="qty2'+id+'" type="hidden" name="item_qty[]" onKeyup="change('+id+', '+unitprice+', '+total_harga+')" value="'+qty+'" readonly required />'+
