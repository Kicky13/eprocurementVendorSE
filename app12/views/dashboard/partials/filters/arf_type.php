<span class="checkbox-list" >
    <div id="heading-filter-po_type" role="tab" class="card-header checkbox-item-list">
        <a data-toggle="collapse" data-parent="#accordionWrap1" href="#accordion-filter-po_type" aria-expanded="false" aria-controls="accordion-filter-po_type" class="h6  collapsed"><?= lang("Type ARF","ARF Type")?></a>
    </div>
    <div id="accordion-filter-po_type" role="tabpanel" aria-labelledby="heading-filter-po_type" class="card-collapse collapse" aria-expanded="false" style="height: 0px;">
        ...
        <div id="filter-content-type">
            <?php foreach ($this->m_dashboard->get_arf_type() as $arf_type) { ?>
                <label class="custom-control ">
                    <input type="checkbox" name="filter[type][]" value="<?= $arf_type->id ?>" data-text="<?= $arf_type->description ?>">
                    <span class="checkbox-label"><?= $arf_type->description ?></span>
                </label>
            <?php } ?>
        </div>
    </div>
