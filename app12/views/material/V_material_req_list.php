    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12 mb-1">
                <h3 class="content-header-title"><?= lang("List Permintaan Material", "List Material Request") ?></h3>
            </div>
            <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-12">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><?= lang("List Permintaan Material", "List Material Request") ?></li>
                    </ol>
                </div>
            </div>
        </div>
                                              ...
                                              <table id="tbl" class="table nowrap table-striped table-bordered table-fixed-column order-column dataex-lr-fixedcolumns table-hover display" width="100%">
                                                  <tfoot>
                                                      <tr>
                                                          ...
                                                          <th>Qty Act</th>
                                                          <th>B/P Warehouse</th>
                                                          <th>To B/P Warehouse</th>
                                                          <th><?= lang("Approval", "Approval") ?></th>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $(document).on('shown.bs.modal', '#modal_approval', function(e){
      ...
      var result;
      $.ajax({
        url: '<?= base_url('material/Material_req_list/get_material_req')?>',
        type: 'POST',
        dataType: 'JSON',
        data: {idnya: idnya}
      })
      ...
      .always(function(res) {
        ...
        if (result = true) {
          ...
          $(".disposal_method").val(res.material_request.disposal_method).select2();
          ...
          if(res.material_request.isclosed == "1"){
            $('#mr_status_approval').html("CLOSE");
          }
    ...
    $(document).on('click', '#btnapprove', function(e){
      ...
      if (confirm('Are you sure to approve?')) {
        ...
        // var xmodalx = '';
        approval_mr('<?= base_url('material/Material_req_list/approve_mr')?>', xmodalx);
      }
    })
    ...
    $(document).on('click', '#btnreject', function(e){
      ...
      swal({
        ...
        if (inputValue == true) {
          ...
          if (notex == '') {
            setTimeout(function(){
              swal("Oops!", "You need to write something !", "warning");
            },1000)
          } else {
            var xmodalx = modal_start($("#modal_approval").find(".modal-content"));
            approval_mr('<?= base_url('material/Material_req_list/reject_mr')?>', xmodalx, notex);
    ...
    $(document).on('click', '.savenote', function(e){
      ...
      if (notes == "") {
        ...
        if (confirm("Save this note, Are you sure ?")) {
          $.ajax({
            url: '<?= base_url('material/Material_req_list/save_note')?>',
            type: 'POST',
            dataType: 'json',
            data: {
              notes: notes,
    ...
    var table = $('#tbl').DataTable({
        ajax: {
            url: '<?= base_url('material/Material_req_list/datatable_mr') ?>',
            dataSrc: ''
        },
        scrollX: true,
        scrollY: '300px',
        ...
        columns: [
            {title: "No", "width": "20px"},
            {title: "<?= lang('Req No', 'Requet No') ?>"},
            {title: "<?= lang('Tanggal', 'Requested Date') ?>"},
            {title: "Material Request Type"},
            {title: "Purpose of Request"},
            {title: "Semic No"},
            {title: "Description Of Unit"},
            {title: "UoM"},
            {title: "Qty Req"},
            {title: "Qty Act"},
            {title: "B/P Warehouse"},
            {title: "To B/P Warehouse"},
            {title: "<?= lang("Approval", "Approval") ?>"},
            {title: "<?= lang("Status", "Status") ?>"},
            {title: "Action"},
        ],
        "columnDefs": [
          {targets:[7,8,9,14], class : 'text-center'}
        ]
    });
  ...
  function get_note(id){
    var result;
    $.ajax({
      url: '<?= base_url('material/Material_req_list/get_note')?>',
      type: 'POST',
      dataType: 'html',
      data: {
        mr_no: id,
  ...
  function get_log(id){
    var result;
    $.ajax({
      url: '<?= base_url('material/Material_req_list/get_log')?>',
      type: 'POST',
      dataType: 'html',
      data: {
        mr_no: id,
