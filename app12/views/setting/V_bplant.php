    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12 mb-1">
                <h3 class="content-header-title"><?= lang("Master BPLANT", "Master BPLANT") ?></h3>
            </div>
            <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-12">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><?= lang("Master BPLANT", "Master BPLANT") ?></li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <form id="formtambah" class="form-horizontal">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label"><?= lang("COMPANY", "COMPANY") ?></label>
                    <div class="controls">
                      <select class="form-control company" name="company" required>
                        <option value=""> Select .. </option>
                        <?php
                        foreach ($company->result_array() as $arr) { ?>
                          <option value="<?= $arr['ID_COMPANY'] ?>"><?= $arr['ID_COMPANY'].' - '.$arr['DESCRIPTION'] ?></option>
                        <?php } ?>
                      </select>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label"><?= lang("ID BPLANT", "ID BPLANT") ?></label>
                    <div class="controls">
                        <input type="text" name="id_acc" id="id_acc" maxlength="12" class="form-control" required>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label class="form-label"><?= lang("ABR", "ABR") ?></label>
                    <div class="controls">
                        <input type="text" name="acc_abr" id="acc_abr" class="form-control" maxlength="4" required>
                    </div>
                </div>
                <div class="form-group">
...
$(document).ready(function() {
  $(document).on('submit', '#formtambah', function (e) {
    e.preventDefault();
    $.ajax({
      url: '<?= base_url('setting/Bplant/add'); ?>',
      dataType: 'json',
      type: 'POST',
      data: $(this).serialize(),
    ...
    $.ajax({
    type: 'GET',
    url: '<?= base_url('setting/Bplant/get/') ?>' + id,
    success: function (res) {
        ...
        $('#id_acc').val(d.ID_BPLANT);
        $('#acc_desc').val(d.BPLANT_DESC);
        $('#acc_abr').val(d.BPLANT_ABR);
        $('.company').val(d.ID_COMPANY);
        if (d.STATUS == "1"){
          document.getElementById("aktif").checked = true;
        } else {
    ...
    var table=$('#tbl').DataTable({
      ajax: {
          url: '<?= base_url('setting/Bplant/show') ?>',
          dataSrc: ''
      },
      scrollX: true,
      scrollY: '300px',
      ...
      columns: [
          ...
          {title: "<center><?= lang('COMPANY', 'COMPANY') ?></center>"},
          {title: "<center><?= lang('ID BPLANT', 'ID BPLANT') ?></center>"},
          {title: "<center><?= lang('Deskripsi', 'Description') ?></center>"},
          {title: "<center><?= lang('ABR', 'ABR') ?></center>"},
