    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12 mb-1">
                <h3 class="content-header-title"><?= lang("Master Mata Uang", "Master Currency") ?></h3>
            </div>
            <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-12">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><?= lang("Master Mata Uang", "Master Currency") ?></li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <form id="formtambah" class="form-horizontal">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label"><?= lang("Kode Mata uang", "Currency Code") ?></label>
                    <div class="controls">
                        <input type="text" name="id_acc" id="id_acc" class="form-control" maxlength="5" required>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label"><?= lang("Deskripsi", "Description") ?></label>
                    <div class="controls">
                        <input type="text" name="acc_desc" id="acc_desc" class="form-control" maxlength="100" required>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="controls">
                        <div class="col-sm-10">
                            <input type="radio" value="1" name="currency_base" id="base_on"> <i></i><?= lang('Aktif', 'Active') ?>
                            &nbsp;&nbsp;&nbsp;
                            <input type="radio" value="0" name="currency_base" id="base_off"> <i></i><?= lang('Nonaktif', 'Nonactive') ?>
                        </div>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
  $(document).on('submit', '#formtambah', function (e) {
    e.preventDefault();
    $.ajax({
      url: '<?= base_url('setting/Currency/add'); ?>',
      dataType: 'json',
      type: 'POST',
      data: $(this).serialize(),
    ...
    function add() {
      ...
      document.getElementById("base_on").checked = true;
      lang();
    }
    ...
    $.ajax({
    type: 'GET',
    url: '<?= base_url('setting/Currency/get/') ?>' + id,
    success: function (res) {
        ...
        if (d.CURRENCY_BASE == "1"){
          document.getElementById("base_on").checked = true;
        } else {
          document.getElementById("base_off").checked = true;
        }
        $('#modal').modal('show');
        lang();
      }
    ...
    var table=$('#tbl').DataTable({
      ajax: {
          url: '<?= base_url('setting/Currency/show') ?>',
          dataSrc: ''
      },
      scrollX: true,
      scrollY: '300px',
      ...
      columns: [
          ...
          {title: "<center><?= lang('Kode Mata Uang', 'Currency Code') ?></center>"},
          {title: "<center><?= lang('Deskripsi', 'Description') ?></center>"},
          {title: "<center><?= lang("Status", "Status") ?></center>", "width": "50px"},
          {title: "<center><?= lang("Based", "Based") ?></center>", "width": "50px"},
