    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12 mb-1">
                <h3 class="content-header-title"><?= lang("Master Acc Sub", "Master Acc Sub") ?></h3>
            </div>
            <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-12">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><?= lang("Master Acc Sub", "Master Acc Sub") ?></li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <form id="formtambah" class="form-horizontal">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label"><?= lang("ID Acc Sub", "ID Acc Sub") ?></label>
                    <div class="controls">
                        <input type="text" name="id_acc" id="id_acc" maxlength="10" class="form-control" required>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label"><?= lang("Deskripsi", "Description") ?></label>
                    <div class="controls">
                        <input type="text" name="acc_desc" id="acc_desc" maxlength="100" class="form-control" required>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="controls">
                        <div class="col-sm-10">
                            <input type="radio" value="0" name="status" id="aktif"> <i></i><?= lang('Aktif', 'Active') ?>
                            &nbsp;&nbsp;&nbsp;
                            <input type="radio" value="1" name="status" id="nonaktif"> <i></i><?= lang('Nonaktif', 'Nonactive') ?>
                        </div>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
  $(document).on('submit', '#formtambah', function (e) {
    e.preventDefault();
    $.ajax({
      url: '<?= base_url('setting/Master_acc_sub/add'); ?>',
      dataType: 'json',
      type: 'POST',
      data: $(this).serialize(),
    ...
    $.ajax({
    type: 'GET',
    url: '<?= base_url('setting/Master_acc_sub/get/') ?>' + id,
    success: function (res) {
        ...
        $('#id_acc').val(d[0].ID_ACCSUB);
        $('#acc_desc').val(d[0].ACCSUB_DESC);
        if (d[0].STATUS == "0"){
          document.getElementById("aktif").checked = true;
        } else {
    ...
    var table=$('#tbl').DataTable({
      ajax: {
          url: '<?= base_url('setting/Master_acc_sub/show') ?>',
          dataSrc: '',
          timeout : 500000
      },
      scrollX: true,
      ...
      columns: [
          ...
          {title: "<center><?= lang('Cost Center', 'Cost Center') ?></center>"},
          {title: "<center><?= lang('ID Acc Sub', 'ID Acc Sub') ?></center>"},
          {title: "<center><?= lang('Deskripsi', 'Description') ?></center>"},
          {title: "<center><?= lang('Posting', 'Posting') ?></center>"},
