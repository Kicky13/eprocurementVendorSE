    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12 mb-1">
                <h3 class="content-header-title"><?= lang("Metode Procurment", "Procurment Method") ?></h3>
            </div>
            <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-12">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><?= lang("Metode Pengadan", "Procurement Method") ?></li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <form id="formtambah" class="form-horizontal">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label"><?= lang("Kode Metode Pengadaan", "Procurement Method Code") ?></label>
                    <div class="controls">
                        <input type="text" name="id_acc" id="id_acc" class="form-control" maxlength="4" required>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label"><?= lang("Deskripsi", "Description") ?></label>
                    <div class="controls">
                        <input type="text" name="acc_desc" id="acc_desc" class="form-control" maxlength="50" required>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="controls">
                        <div class="col-sm-10">
                            ...
                            &nbsp;&nbsp;&nbsp;
                            <input type="radio" value="0" name="status" id="nonaktif"> <i></i><?= lang('Nonaktif', 'Inactive') ?>
                        </div>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
  $(document).on('submit', '#formtambah', function (e) {
    e.preventDefault();
    $.ajax({
      url: '<?= base_url('setting/Pmethod/add'); ?>',
      dataType: 'json',
      type: 'POST',
      data: $(this).serialize(),
    ...
    $.ajax({
    type: 'GET',
    url: '<?= base_url('setting/Pmethod/get/') ?>' + id,
    success: function (res) {
        ...
        $('#id_acc').val(d.ID_PMETHOD);
        $('#acc_desc').val(d.PMETHOD_DESC);
        if (d.STATUS == "1"){
          document.getElementById("aktif").checked = true;
        } else {
    ...
    var table=$('#tbl').DataTable({
      ajax: {
          url: '<?= base_url('setting/Pmethod/show') ?>',
          dataSrc: ''
      },
      scrollX: true,
      scrollY: '300px',
      ...
      columns: [
          ...
          {title: "<center><?= lang('Kode Metode Pengadaan', 'Procurement Method Code') ?></center>"},
