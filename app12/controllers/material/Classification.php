...
class Classification extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('material/M_classification')->model('vendor/M_vendor');
    }

    public function index() {
        $tamp = $this->M_classification->show();
        ...
        $data['menu'] = $dt;
        $this->template->display('material/V_classification', $data);
    }
    ...
    public function filter_data() {
        $res = $this->M_classification->filter_data($_POST);
        $dt = array();
        foreach ($res as $k => $v) {
            $dt[$k][0] = $k + 1;
    ...
    public function show() {
        $data = $this->M_classification->show();
        $dt = array();
        foreach ($data as $k => $v) {
            $dt[$k][0] = $k + 1;
            $dt[$k][1] = stripslashes($v->MATERIAL_GROUP);
            $dt[$k][2] = stripslashes($v->DESCRIPTION);
            $dt[$k][3] = stripslashes($v->TYPE);
            if ($v->STATUS == 1) {
                $dt[$k][4] = "Active";
            } else {
                $dt[$k][4] = "Nonactive";
    ...
    public function get($id) {
        echo json_encode($this->M_classification->show(array("ID" => $id)));
    }
    ...
    public function change() {
        ...
        if ($_POST['id'] == null) {//add data
            $data['CREATE_BY'] = 1;
            $cek = $this->M_classification->show(array("MATERIAL_GROUP" => strtoupper(stripslashes($_POST['mat_group']))));
            if (count($cek) == 0) {//cek ketersediaan data
                $q = $this->M_classification->add($data);
            } else {
                echo "Tipe Materaial Telah Digunakan";
                exit;
            }
        } else {//update data
            $q = $this->M_classification->update($_POST['id'], $data);
        }
