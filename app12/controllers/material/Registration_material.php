...
class Registration_material extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('material/M_material_registration')->model('material/M_mregist_approval')->model('vendor/M_vendor');
    ...
    public function index() {
      ...
      $data['menu'] = $dt;
      $this->template->display('material/V_registration_material', $data);
    }
    ...
    public function show() {
        $data = $this->M_material_registration->show();
        $dt = array();
        foreach ($data as $k => $v) {
            $dt[$k][0] = $k + 1;
            ...
            $dt[$k][7] = "<a class='btn btn-primary btn-sm' title='Update' href='javascript:void(0)' onclick='update(" . $v->ID . ")'><i class='fa fa-edit'></i></a>";
        }
        echo json_encode($dt);
    }
...
//        strtoupper($string)
        ...
        if ($_POST['id'] == null) {//add data
            $data['CREATE_BY'] = 1;
            $cek = $this->M_material_registration->show(array("MATERIAL" => strtoupper(stripslashes($_POST['mat_material']))));
            if (count($cek) == 0) {//cek ketersediaan data
                $q = $this->M_material_registration->add($data);
            } else {
                echo "Tipe Materaial Telah Digunakan";
                exit;
            }
        } else {//update data
            $q = $this->M_material_registration->update($_POST['id'], $data);
        }
    ...
    public function save_material_requestor() {
        ...
        if ($img1['error'] != 4 && $img1['tmp_name'] !== '') {
            $fileurl1 = image_upload($img1["tmp_name"], $img1["name"], $img1["type"], $img1["size"], $path_ori, $path_sml);
            $array_data['IMG1_URL'] = $fileurl1;
            $array_data['IMG3_URL'] = $fileurl1;
        }

        if ($img2['error'] != 4 && $img2['tmp_name'] !== '') {
            $fileurl2 = image_upload($img2["tmp_name"], $img2["name"], $img2["type"], $img2["size"], $path_ori, $path_sml);
      			$array_data['IMG2_URL'] = $fileurl2;
      			$array_data['IMG4_URL'] = $fileurl2;
        }

        if ($img3['error'] != 4 && $img3['tmp_name'] !== '') {
            $fileurl3 = file_uploads($img3["tmp_name"], $img3["name"], $img3["type"], $img3["size"], $path_file, $path_sml);
      			$array_data['FILE_URL'] = $fileurl3;
      			$array_data['FILE_URL2'] = $fileurl3;
        } else {
          $array_data['FILE_URL'] = "-";
          $array_data['FILE_URL2'] = "-";
        }

        if (!empty($_POST['material_id'])) {
            $material = $_POST['material_id'];
            $result = $this->M_material_registration->update_cond(array('MATERIAL' => $_POST['material_id']), $array_data);
        } else {
            $result = $this->M_material_registration->add($array_data);
            if ($result !== false) {
                $material = $result;
                $result = $this->M_material_registration->add_approval($material, $_SESSION['ID_USER']);
            }
        }
        ...
        if ($result !== false) {
            $dest = $this->M_material_registration->get_email_dest($material);
            if ($dest !== false) {
                $rec = $dest[0]['recipients'];
                $rec_role = $dest[0]['rec_role'];
                $user = $this->M_material_registration->get_email_rec($rec, $rec_role);
    ...
    public function datatable_registrasi_material() {
      ...
      foreach ($data as $arr) {
        ...
        if (strpos($arr['description'], "DATA DITOLAK") === FALSE) {
          ...
          $disabled = "disabled";
          $class = 'danger';
          $str = '<button onclick="update('.$arr['material_id'].')" class="btn btn-warning btn-sm" title="Edit">Edit</button> <button type="button" class="btn btn-danger btn-sm" data-id="'.$arr['material_id'].'" id="delete" title="Delete">Delete</button>';
        }
