...
class Job_title extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('setting/M_job_title')->model('vendor/M_vendor');
    ...
    public function index(){
      ...
      $data['menu'] = $dt;
      $this->template->display('setting/V_job_title', $data);
    }

    public function show() {
      $data = $this->M_job_title->show();
      $result = array();
      $no = 1;
    ...
    public function get($id) {
        echo json_encode($this->M_job_title->show($id));
    }
    ...
    public function add() {
      ...
      $qq = $this->db->query("SELECT * FROM m_job_title WHERE id='".$id."'");
      $num = $qq->num_rows();

      if ($id != "") {
        // $data['CHANGED_BY'] = $this->session->userdata['ID_USER'];
        // $data['CHANGED_ON'] = date("Y-m-d H:i:s");
        $query = $this->M_job_title->update($id, $data);
        ...
        if ($num > 0) {
          ...
          $query = $this->M_job_title->simpan($data);
