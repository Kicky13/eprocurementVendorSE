...
class M_type extends CI_Model {
    ...
    public function show($where = null) {
        ...
        $data = $this->db->from('m_material_type')->order_by('CREATE_TIME DESC')->get();
        return $data->result();
    }

    public function add($data) {
        $data = $this->db->insert('m_material_type', $data);
//        echo $this->db->last_query();exit;
        return $data;
    }

    public function update($id, $data) {
        return $this->db->where('ID', $id)->update('m_material_type', $data);
    }
    ...
    public function filter_data($data) {
        ...
        $this->db->select("ID,MATERIAL_TYPE,DESCRIPTION,STATUS")
                ->from("m_material_type");        
