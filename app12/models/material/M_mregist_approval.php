...
class M_mregist_approval extends CI_Model {
    ...
    public function m_gl_class(){
      $q = $this->db->select("*")->from("m_gl_class")->where("status", "1")->order_by("CODE", "ASC");
      $result = $this->db->get();
      return $result->result_array();
    }
    ...
    public function get_list() {
        $query = $this->db->query("
            SELECT b.id, b.id_user, b.user_roles, b.material_id, m.REQUEST_NO as request_no, r.description as role, m.UOM as user_uom, m.UOM1 as spec_uom, a.sequence, b.status_approve,
                CASE
            ...
            FROM (
                ...
                WHERE (status_approve = 0 OR status_approve = 2) AND extra_case = 0 and sequence > 1
                GROUP BY material_id
            ) a
            JOIN t_approval_material b ON b.material_id = a.material_id AND b.sequence = a.sequence AND b.sequence<>1
            ...
            GROUP BY b.id, b.id_user, b.user_roles, b.material_id, request_no, role, m.UOM, m.UOM1, b.description, m.MATERIAL_NAME, m.DESCRIPTION1, m.DESCRIPTION, a.sequence, b.status_approve");
    ...
    public function datatable_logistic_specialist($where = null) {
        ...
        $query = $this->db->query("SELECT * FROM t_approval_material WHERE id='".$where."'");
        return $query->result_array();
    ...
    public function get_cur_seq($id) {
        $res = $this->db->query("
                ...
                FROM (
                    SELECT min(sequence) as sequence, material_id
                    FROM t_approval_material
                    WHERE material_id = '".$id."' AND status_approve = 0
                    GROUP BY material_id
                ) a
    ...
    public function get_cur_seq_list($id) {
        $res = $this->db->query("
                ...
                FROM (
                    SELECT min(sequence) as sequence, material_id
                    FROM t_approval_material
                    WHERE material_id = '".$id."' AND status_approve = 1
                    GROUP BY material_id
                ) a
                JOIN t_approval_material b ON b.material_id = a.material_id AND b.sequence = a.sequence
            ");
    ...
    public function save_registrasi_cataloguing($data, $idmat, $fedit, $seq) {
        ...
        if ($fedit == '1') {
            ...
            if ($data['MATERIAL_CODE'] != "") {
                ...
                $this->db->where("MATERIAL", $idmat);
                $this->db->update("m_material", $update_code);
            }
        }
        ...
        $this->db->where("material_id", $idmat);
        $this->db->where("status_approve", "2");
        $this->db->update("t_approval_material", $status_reject);

        $this->db->where("material_id", $idmat);
        $this->db->where("sequence", $seq);
        $this->db->update("t_approval_material", $data_squence);
    ...
    public function reject_request_material($data){
      ...
      $this->db->where("material_id", $data['idnya']);
      $this->db->update("t_approval_material", $upd_status_0);

      $this->db->where("id", $data['m_approval_id']);
      $this->db->update("t_approval_material", $upd_status_reject);

      $data_res2 = array(
        'ID_MATERIAL' => $data['idnya'],
        'NOTE' => 'CATALOG NUMBER REQUEST REJECTED BY '.$this->session->userdata['NAME'],
        "CREATE_BY" => $data['user'],
        "CREATE_TIME" => date("Y-m-d H:i:s"),
      );
    ...
    public function rollback_apprvl($material){
      ...
      $sel_apprv = $this->db->query("SELECT MAX(sequence) as sequence, id FROM t_approval_material WHERE material_id='".$material."'");
      $this->db->where("material_id", $material);
      $this->db->where("sequence", $sel_apprv->row()->sequence);
      $this->db->update("t_approval_material", $update2);
